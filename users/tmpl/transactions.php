...
<div class="table-rep-plugin">
    <div class="table-responsive b-0" >
        <?php include ('../db.php');
            ...
            and transactions.estate = flats.estate_code and transactions.estate = '".$_SESSION['estate']."'";
			...
			if($_SESSION['admin_type'] == 'flat') { 
			   $sql = "SELECT * FROM transactions where estate='".$_SESSION['estate']."' AND flat='".$_SESSION['flat_no']."' AND block='".$_SESSION['block_no']."' ORDER BY transaction_date";
			}
			//echo $sql; 
			$result = $con->query($sql);
			if ($result->num_rows > 0) { ?>
			<table id="example" class="table  table-bordered">
			<thead><tr class="titles"><th>Transaction ID</th><th>Meter PAN</th> <th>Transaction Date</th><th>Value</th><th>Token</th><th>Units</th><th>Transaction By</th><?php if($_SESSION['admin_type'] != 'flat') { ?><th>Action</th><?php } ?></tr></thead>
			  <tbody> <?php while($row = $result->fetch_assoc()) { ?>
					<tr><td><?php echo $row['transaction_id']; ?></td><td><?php echo $row['meter_pan']; ?></td><td><?php echo $row['transaction_date']; ?></td><td><?php echo $row['amount']; ?></td><td><?php echo $row['token']; ?></td><td><?php echo $row['units']; ?></td>
