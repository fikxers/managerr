                  ...
                  <?php include ('../db.php');
					...
					if ($result->num_rows > 0) { ?>
					  <table id="tech-companies-1" class="table editable-table table-bordered table-striped m-b-0">
                        <thead>
                          <tr class="titles">
                          	<th>Room No.</th><th>No. of Students</th><th>Fee</th><th>Action</th>
							<?php if($_SESSION['admin_type']=='admin'){echo 'Estate';} ?>
                          </tr>
                        </thead>
                        <tbody> <?php
						while($row = $result->fetch_assoc()) { ?>
						<tr><td contenteditable="true"><?php echo $row['room_no']; ?></td>
							<td contenteditable><?php echo $row['no_of_students']; ?></td>
							<td contenteditable><?php echo $row['fee']; ?></td>
