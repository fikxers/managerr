...
<?php
  ...
  include ('../db.php');
  $id = $_GET['id'];	$address = $_GET['address'];	$flats = $_GET['flats'];	
  $blocks = $_GET['blocks'];	$name = $_GET['name'];
  ...
  if (isset($_POST['update'])){
	...
	$query = "UPDATE estates set estate_name='".$name."',address='".$address."',no_of_blocks=$blocks,no_of_flats=$flats WHERE estate_code = '".$id."'";
	$result2 = mysqli_query($con,$query); 
	//echo "<script>alert('".$query."');</script>";
	if($result2){
	  echo "<script>alert('Estate updated successfully.');</script>";
	  echo "<script type='text/javascript'>window.top.location='view_estates.php';</script>"; exit;
	}
	else{
	  echo "<script>alert('Error updating estate.');</script>";
	  echo "<script type='text/javascript'>window.top.location='view_estates.php';</script>"; exit;
	}
  }
  else if (isset($_POST['cancel'])){
	 echo "<script type='text/javascript'>window.top.location='view_estates.php';</script>";
  }
  else{
  ?>
