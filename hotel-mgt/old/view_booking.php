...
<?php
  ...
  if (isset($_GET['room_number'])){
    ...
    $sql = "SELECT t.price_per_night price FROM room JOIN room_type t USING(type_id)";
    ...
    if($task == 'book' || $task == 'edit'){
      switch ($task) {
        case "book":
          ...
          $msg = "Guest: $guest | Checkin: ".$checkin." | Checkout: ".$checkout." | No. of days: ".$no_of_days." | Total fee: ".currency_format($total_price)."<br>";
          ...
          $add_booking = "INSERT INTO `booking`(`guest_id`, `room_id`, `checkin_date`, `checkout_date`, `total_price`) VALUES ($guest,$room_no,'".$checkin."','".$checkout."',$total_price)";
                    ...
                    <div class="box box-primary">
                        <div class="box-body">
                            ...
                            <?php 
                              ...
                              $booking = "SELECT * FROM `booking` WHERE booking_id = (SELECT MAX(booking_id) from booking WHERE room_id=1)";
                            ?>
                            <h5>Checkin Date: </h5>
                            <h5>Checkout Date: </h5>
                        </div>
