<?php require('auth.php'); $title ='Notices'; ?>
    ...
    <?php
		  ...
		  if (isset($_POST['send_notice'])){
			...
			$query = "INSERT INTO `notices`(`hostel`, `message`, `subject`, `send_date`) VALUES ('".$_SESSION['hostel']."','".$message."','".$subject."','".$trn_date."')";
			$result = mysqli_query($con,$query);
			if($result){
			  echo "<script>alert('Notice sent.');</script>";
			  echo "<script type='text/javascript'>window.top.location='notices.php';</script>"; exit;
			}
			else{
			  echo "<script>alert('Notice could not be sent.');</script>";
			  echo "<script type='text/javascript'>window.top.location='notices.php';</script>"; exit;
			}
		  }
		  else{
                  ...
                  <?php include ('../db.php');
					...
					if ($result->num_rows > 0) { ?>
					  <table id="tech-companies-1" class="table editable-table table-bordered table-striped m-b-0">
                        <thead>
                          <tr class="titles">
                          	<th>Subject</th><th>Message</th><th>Date Sent</th><!--<th>Action</th>-->
