<?php require('auth.php'); $title ='Work Orders'; 
      if($_SESSION['admin_type']=='hmgr') {
      	...
      	$sql = "SELECT * FROM student_requests where hostel = '".$_SESSION['hostel']."' and status != 'Pending'";
        $total_cost_sql = "SELECT SUM(fee) as sm FROM student_requests WHERE hostel = '".$_SESSION['hostel']."'";
        $total_cost_res = mysqli_query($con,$total_cost_sql) or die(mysql_error());
        $total_cost = $total_cost_res->fetch_object()->sm;
      } 
    ...
    ?>
        <div class="row">
            ...
            <div class="card m-b-30">
              <div class="card-body">
                <h4 class="mt-0 header-title">Total maintenance cost: &#8358;<?php echo $total_cost; ?></h4>
                <div class="table-rep-plugin">
                  ...
                  <?php 
                        ...
                        <thead>
                          <tr class="titles">
                          	<th>Service</th><th>Equipment</th><th>Service Date</th><th>Info</th><th>Room</th><th>Status</th><th>Fikxer</th><th>Feedback</th><th>Fee</th><th>Action</th>
